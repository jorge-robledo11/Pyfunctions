...
class FillMissingValuesTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X:pd.DataFrame):
        ...
        X_no_missing = X_no_missing.replace({
            'ERROR': np.nan,
            '': np.nan,
            'None': np.nan,
            'n/a': np.nan,
...
class DateColumnsTransformer(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X:pd.DataFrame):
        ...
        # Si no se especifican las columnas, busca columnas que contengan ciertas palabras clave
        if self.date_columns is None:
            date_keywords = ['fecha', 'date', 'tiempo', 'time']
            self.date_columns = [col for col in X.columns if any(keyword in col for keyword in date_keywords)]

        # Convierte las columnas seleccionadas a datetime
        for col in self.date_columns:
            if col in X_transformed.columns:
                X_transformed[col] = pd.to_datetime(X_transformed[col], format=self.format, errors='coerce')
...
#         ('DropDuplicatedTransformer', DropDuplicatedRowsTransformer()),
#         ('FillMissingValuesTransformer', FillMissingValuesTransformer()),
#         ('DateColumnTransformer', DateColumnsTransformer(df.filter(regex='fecha|date|tiempo|time').columns)),
